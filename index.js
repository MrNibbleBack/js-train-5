...
function findMinMax([firstElement, ...restElements]) {
  ...
  for (let i = 0; i <= restElements.length; i++) {
    if (valueMin > restElements[i]) {
      valueMin = restElements[i];
    }
    if (valueMax < restElements[i]) {
      valueMax = restElements[i];
    }
  }
  return console.log(`Min = ${valueMin}, Max = ${valueMax}`);
}
...
function multiplyByIndex(arr) {
  ...
  let indexMultiply = new Array(arr.length);
  for (const index in arr) {
    indexMultiply[index] = arr[index] * index;
  }
...
function replaceNumbers(arr) {
  ...
  // Повертаємо масив результату
  let replaceNums = new Array(arr.length);
  for (const index in arr) {
    if (arr[index] > 10) {
      replaceNumbers[index] = "Greater than 10";
    } else {
      replaceNumbers[index] = "Less than or equal to 10";
    }
  }
